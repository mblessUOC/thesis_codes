...
**Description:** Convolutional Variational AutoEncoder (VAE) to trained on PCA images from miRNA dataset without grouping.
...
**Conclusion**:
*Epoch 258: - loss_vae: 155.5221 - reconstruction_loss: 154.3540 - kl_loss: 2.0878*
...
encSub1 = layers.Conv2D(kernel_sizes[0], 3, strides=1, padding="same",use_bias=use_bias, name="encSub1_conv2D.1.0")(miRNA_encSub1_inputs)
encSub1 = layers.BatchNormalization(name="encSub1_conv2D.1.2")(encSub1)
encSub1 = layers.LeakyReLU(name="encSub1_conv2D.1.3")(encSub1)

encSub1 = layers.MaxPool2D(pool_size=down_ratio, name="encSub1_MaxPool.1")(encSub1) # MaxPool -1-

encSub1 = layers.Conv2D(kernel_sizes[1], 3, strides=1, padding="same",use_bias=use_bias, name="encSub1_conv2D.2.0")(encSub1)
encSub1 = layers.BatchNormalization(name="encSub1_conv2D.2.1")(encSub1)
encSub1 = layers.LeakyReLU(name="encSub1_conv2D.2.2")(encSub1)

miRNA_encSub1_outputs = layers.MaxPool2D(pool_size=down_ratio,name="encSub1_MaxPool.2")(encSub1) # MaxPool -2-
...
miRNA_inputs = keras.Input(shape=(img_dim, img_dim, 1),name="x_miRNA_input")
encSub2_inputs = miRNA_encoder_sub1(miRNA_inputs)
...
encoder = keras.Model(miRNA_inputs, [z_mean,z_log_var], name="encoder")
...
decSub2 = layers.Conv2DTranspose(kernel_sizes[2], 3, activation="relu", strides=up_ratio*2, padding="same",use_bias=use_bias,name="decSub2_convTrans2D.1.0")(decSub2)
...
miRNA_decoder_sub1_output = decoder_sub1(decSub2_outputs)

decoder = keras.Model(z_sample, miRNA_decoder_sub1_output, name="decoder")
...
checkPath = '/content/drive/MyDrive/Maxi Bless/VAEs/Datos_Colab/Weights/VAE_V6/model.V6.1_miRNA.h5'
...
# Training logger
#------------------------------------------------
logPath='/content/drive/MyDrive/Maxi Bless/VAEs/Datos_Colab/Run/VAE_V6/training.V6.1_miRNA.csv'
...
for file_data,file_metadata in zip(list_of_data,list_of_metadata):
    ...
    #print("temp shape:",tempMeta.shape[1])
    if tempMeta.shape[1] != metadata_array.shape[1]:
        tempMeta=np.delete(tempMeta,[18,19,26],axis=1)
...
df_labels.value_counts().to_csv('/content/drive/MyDrive/Maxi Bless/VAEs/Datos_Colab/data/other/sampleFrequency_VAE_V6.1_miRNA.csv', index=True, header=True)
...
image_index = random.sample(range(0,data_train.shape[0]),10)
plt.figure(figsize=(30, 15))
plt.rcParams.update({'font.size': 16})

#nDim1=data_train.shape[1]
#nDim2=data_train.shape[2]
...
for i,index in enumerate(image_index):
    ...
    plt.title(metadata_array[index,18]+'\n'+metadata_array[index,24])
...
VAE_miRNA.compile(optimizer=keras.optimizers.Adam(0.0001))
VAE_miRNA.metrics_names
VAE_miRNA.fit(data_train,
              epochs=EPOCH,
...
#Creo el modelo
VAE_miRNA=VAE(encoder, decoder)
VAE_miRNA.built = True
VAE_miRNA.load_weights(filepath = checkPath)
#Initiate from beginning
VAE_miRNA.compile(optimizer=keras.optimizers.Adam(0.0001))
...
z_mean,var_mean=VAE_miRNA.encoder.predict(data_reduced)
z_sample=VAE_miRNA.sampler(z_mean,var_mean)  
reconstruct = VAE_miRNA.decoder.predict(z_sample)

plt.imshow(np.squeeze(reconstruct[0]), cmap="gist_rainbow")
plt.axis('off')
plt.title(metadata_array[image_index[0],24]+'\n'+metadata_array[image_index[0],18])
...
print("Layers in VAE_miRNA\n")
for i,layer in enumerate(VAE_miRNA.layers):
    #print(layer.name, layer)
    print(i,layer.name)
print("\n*--------------------------*\n")
print("Layers in submodel Encoder\n")
for i,layer in enumerate(VAE_miRNA.layers[0].layers):
    #print(layer.name, layer)
    print(i,layer.name)
print("\n*--------------------------*\n")
# Itero sobre las capas de decSub1
print("Layers in submodel Decoder\n")
for i,layer in enumerate(VAE_miRNA.layers[1].layers):
    #print(layer.name, layer)
    print(i,layer.name)

#Extraigo subsubmodelo encoder_sub1 y guardo sus pesos
saveSubEncoder_weight="/content/drive/MyDrive/Maxi Bless/VAEs/Datos_Colab/Weights/VAE_V6/SubEncoder_weight_miRNA_V6.1.h5"
check_if_duplicated(saveSubEncoder_weight)
print("Modelo: "+VAE_miRNA.name)
print("  -> capa 0: "+VAE_miRNA.layers[0].name)
for i,layer in enumerate(VAE_miRNA.layers[0].layers):
    print("       ->",i,"Submodelo:",layer.name)

print("\n *Saving weights of: "+VAE_miRNA.layers[0].layers[1].name)
VAE_miRNA.layers[0].layers[1].save_weights(saveSubEncoder_weight)

#Extraigo subsubmodelo decoder_sub1 y guardo sus pesos
saveSubEncoder_weight="/content/drive/MyDrive/Maxi Bless/VAEs/Datos_Colab/Weights/VAE_V6/SubDecoder_weight_miRNA_V6.1.h5"
#check_if_duplicated(saveSubEncoder_weight)
print("Modelo: "+VAE_miRNA.name)
print("  -> capa 1: "+VAE_miRNA.layers[1].name)
for i,layer in enumerate(VAE_miRNA.layers[1].layers):
    print("       ->",i,"Submodelo:",layer.name)

print("\n *Saving weights of: "+VAE_miRNA.layers[1].layers[9].name)
VAE_miRNA.layers[1].layers[9].save_weights(saveSubEncoder_weight)

VAE_miRNA.layers[0].layers[1].summary()

VAE_miRNA.layers[1].layers[9].summary()
